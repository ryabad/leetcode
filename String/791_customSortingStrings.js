...
function customSortString(order, s) {
  ...
  for (let value in orderCount) {
    while (orderCount[value] !== 0) {
      result += value;
      orderCount[value]--;
    }
  }

  const filteredArrayOfString = sortString
    .split("")
    .filter((value) => result.includes(value) === false)
    .join("");

  return result + filteredArrayOfString;
}
